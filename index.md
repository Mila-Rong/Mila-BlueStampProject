...
# Starter Project
For my starter project, I built a Arduino starter that has ability to detect whether there is an object in front and the distance from itself. This happens by using a ultrasounic sensor and an LED. To measure the realtime distance, it will time the ultrasound emmited travel back and forth from the object. The ultrasound velocity is known as 340m/s, allow it to calculate the the distance(S = vt), and shown through serial monitor. To control the LED turn on when the object is near, Arduino IDE is utilized to add code and ensure the LED turn on when the distance is less than 6cm, off when the distance is greater than 6cm.
One of the challenge I met is that the LED is always off. After checking the circuit board connection and instrument performance, I found I made a simple mistake that I made the positive electrode connected to ground. Although it worked, another seemingly insurmountable challenge emerges. The LED is on when there is nothing in front. The first step is to change the digital write order from the mistake critical distance to ledpin. The other one is to find the reason the LED is always on. Then I found the sencer measure the distance as 0 if an object is farther than it max distance, such as when there is nothing. Finally, I add code about not turn LED on when the distance is 0. However, this project allowed me to become comfortable using Arduino systems and related tools.
## Code
